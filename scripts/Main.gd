...
func _ready():
	...
	monitor_area.connect("input_event", self, "_on_monitor_clicked")
...
func _process(delta):
	...
	camera.zoom = camera.zoom.linear_interpolate(target_zoom, zoom_speed * delta)
	...
	camera.global_position = camera.global_position.linear_interpolate(target_pos, zoom_speed * delta)
	
	# Hide character when zoomed (you ARE the character)
	character.modulate.a = lerp(character.modulate.a, 0.0 if is_zoomed_in else 1.0, 5.0 * delta)

func _input(event):
	# ESC to zoom out
	if event.is_action_pressed("zoom_out") and is_zoomed_in:
		zoom_out()
